...
const UserInfo: React.FC<UserInfoProps> = ({chartOptions}) => {
	...
	const paginatedData = userHistory?.slice(currentPage * ITEMS_PER_PAGE, (currentPage + 1) * ITEMS_PER_PAGE);
    ...
    return (
        <div
			...
			>
				...
				<section className="flex flex-col space-y-8 mt-5 2xl:relative 2xl:bottom-[291px] 2xl:w-[960px]  overflow-x-auto ">
					<h2 className="font-semibold text-xl sm:text-2xl">Transaction History</h2>
					<div className="relative overflow-x-auto sm:h-[460px]">
							...
							<tbody className="divide-y-8 divide-transparent w-full h-60 md:table-row-group space-y-4">
								{
									...
									>
										...
										>
											<span
												className="font-medium text-sm tracking-tight text-gray-800"
												>{history.metri_equivalent} <span className="text-xs">MTR</span></span
											>
										</td>
										<td
											className="px-6 py-4"
										>
											<span
												className="font-medium text-sm tracking-tight text-gray-800"
												>{history.amount} <span className="text-xs">{history.crypto_type}</span></span
											>
										</td>
										<td className=" px-6 py-4">
						...
						<div className="flex justify-between mt-4 sm:absolute sm:w-full sm:bottom-0" id="arrows">
							<button
							...
							{Array.from({ length: totalPages }, (_, index) => (
								...
								className={`px-4 py-2 ${currentPage === index ? 'bg-lightGreen text-white' : 'bg-gray-200 text-gray-700'} rounded`}
								>
								{index + 1}
								</button>
							))}
